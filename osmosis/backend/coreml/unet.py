...
class CrossAttention(nn.Module):
    ...
    def forward(self, hidden_states, context=None, mask=None):
        ...
        if ATTENTION_IMPLEMENTATION_IN_EFFECT == AttentionImplementations.ORIGINAL:
            ...
            attn = attn.contiguous().view(bs, self.heads * self.dim_head, 1, -1)
            ...
            mh_q = [
                q[:, head_idx * self.dim_head : (head_idx + 1) * self.dim_head, :, :]
                for head_idx in range(self.heads)
            ]  # (bs, dim_head, 1, max_seq_length) * heads

            k = k.transpose(1, 3)
            mh_k = [
                k[:, :, :, head_idx * self.dim_head : (head_idx + 1) * self.dim_head]
                for head_idx in range(self.heads)
            ]  # (bs, max_seq_length, 1, dim_head) * heads

            mh_v = [
                v[:, head_idx * self.dim_head : (head_idx + 1) * self.dim_head, :, :]
                for head_idx in range(self.heads)
            ]  # (bs, dim_head, 1, max_seq_length) * heads
        ...
        *,
        ...
        if temb_channels is not None:
            self.time_emb_proj = torch.nn.Conv2d(
                temb_channels, out_channels, kernel_size=1
            )
        else:
...
class FeedForward(nn.Module):
    def __init__(self, dim, dim_out=None, mult=4, glu=False):
        ...
        self.net = nn.Sequential(
            ...
            nn.Identity(),
            nn.Conv2d(
                inner_dim, dim_out if dim_out is not None else dim, kernel_size=1
            ),
        )
...
class TimestepEmbedding(nn.Module):
    def __init__(self, channel, time_embed_dim, act_fn="silu"):
        ...
        if act_fn == "silu":
            self.act = nn.SiLU()
        self.linear_2 = nn.Conv2d(time_embed_dim, time_embed_dim, kernel_size=1)
